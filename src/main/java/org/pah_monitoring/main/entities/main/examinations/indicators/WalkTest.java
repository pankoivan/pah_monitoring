...
public class WalkTest implements Indicator {
    ...
    public enum Breathlessness {
        ...
        SCORE_0("Отсутствует (0)", 0),

        SCORE_0_5("Очень-очень лёгкая (0.5)", 0.5),

        SCORE_1("Очень лёгкая (1)", 1),

        SCORE_2("Лёгкая (2)", 2),

        SCORE_3("Умеренная (3)", 3),

        SCORE_4("В некоторой степени тяжёлая (4)", 4),

        SCORE_5("Тяжёлая (5)", 5),

        SCORE_6("Тяжёлая (6)", 6),

        SCORE_7("Очень тяжёлая (7)", 7),

        SCORE_8("Очень тяжёлая (8)", 8),

        SCORE_9("Очень-очень тяжёлая (9)", 9),

        SCORE_10("Максимальная (10)", 10);
